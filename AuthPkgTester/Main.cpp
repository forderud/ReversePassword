...
)
{
    ...
    if (!LogonUser(
        ...
        &hToken))
    {
        wprintf(L"ERROR: LogonUser faile with err %u\n", GetLastError());
        goto Cleanup;
    }
    ...
    if (!bResult) {
        wprintf(L"ERROR: CreateProcessAsUser faile with err %u\n", GetLastError());
        abort();
    }
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    const wchar_t cmdLine[] = L"C:\\Windows\\System32\\cmd.exe";
    StartInteractiveClientProcess(username, nullptr, password, (wchar_t*)cmdLine);
}
